...
function renderWeekly(){
  ...
  for(let hr=SH();hr<EH();hr++){
    ...
    for(let d=0;d<7;d++){
      ...
      let eh='';
      // 이 날 전체 이벤트 겹침 레이아웃 계산 (hr 루프 밖에서 한 번만)
      const _wEvs=getEvs(day).map(e=>({...e,_type:'ev',startMin:(e.startH-SH())*60+e.startM,endMin:(e.endH-SH())*60+e.endM}));
      const _wCls=getCls(day).map(e=>({...e,_type:'cls',startMin:(e.startH-SH())*60+e.startM,endMin:(e.endH-SH())*60+e.endM}));
      const _wAll=calcOverlapLayout([..._wEvs,..._wCls]);
...
function calcOverlapLayout(items){
  ...
  for(let i=0;i<res.length;i++){
    ...
    for(let j=i+1;j<res.length;j++){
      if(used[j])continue;
      // 그룹 내 어떤 아이템과라도 겹치면 같은 그룹
      if(grp.some(k=>res[k].startMin<res[j].endMin&&res[j].startMin<res[k].endMin)){
  ...
  for(const grp of groups){
    const cols=[]; // cols[i] = 해당 컬럼의 마지막 endMin
    for(const idx of grp){
      let placed=false;
      for(let c=0;c<cols.length;c++){
        if(cols[c]<=res[idx].startMin){
          res[idx].col=c;cols[c]=res[idx].endMin;placed=true;break;
        }
      }
      if(!placed){res[idx].col=cols.length;cols.push(res[idx].endMin);}
    }
    const totalCols=cols.length;
    for(const idx of grp)res[idx].totalCols=totalCols;
  }
  return res;
}
