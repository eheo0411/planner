...
function calcOverlapLayout(items){
  // items: [{startMin, endMin, ...기타}]
  // 반환: 각 item에 {col, totalCols} 추가
  if(!items.length)return[];
  // startMin 순 정렬
  const res=items.map((e,i)=>({...e,_i:i,col:0,totalCols:1}))
    .sort((a,b)=>a.startMin-b.startMin);

  // 각 아이템마다 동시에 겹치는 아이템 집합 계산
  for(let i=0;i<res.length;i++){
    // i와 실제로 겹치는 것들 (직접 겹침만)
    const peers=[i];
    for(let j=0;j<res.length;j++){
      if(i===j)continue;
      if(res[i].startMin<res[j].endMin-1 && res[j].startMin<res[i].endMin-1)peers.push(j);
    }
    // 이 피어 그룹 안에서 컬럼 배정 (greedy)
    // 이미 배정된 col 재사용, 없으면 새 col
    const usedCols=peers.filter(j=>j<i).map(j=>res[j].col);
    let col=0;
    while(usedCols.includes(col))col++;
    res[i].col=col;
    // totalCols: 피어 그룹 전체가 동시에 차지하는 최대 col+1
    const maxCol=Math.max(...peers.map(j=>res[j].col));
    const total=maxCol+1;
    // 피어 모두에게 totalCols 갱신 (최대값으로)
    peers.forEach(j=>{if(res[j].totalCols<total)res[j].totalCols=total;});
  }
  return res;
}
