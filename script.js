...
function renderTT(){
  ...
  let minH=SH(),maxH=EH();
  classes.forEach(c=>{minH=Math.min(minH,c.startH);maxH=Math.max(maxH,c.endH+1)});
  ...
  for(let h=minH;h<maxH;h++){
    ...
    for(let d=1;d<=5;d++){
      const cls=classes.find(c=>c.days.includes(d)&&c.startH===h);
      let inner='';
      if(cls){const dur=(cls.endH-cls.startH)*60+(cls.endM-cls.startM);inner=`<div class="ttcls" style="height:${Math.round(dur/60*CH)-2}px;background:${h2r(cls.color,.15)};border-left-color:${cls.color};color:${cls.color}" onclick="openClsModal(${cls.id})"><div style="word-break:keep-all;overflow-wrap:break-word;white-space:normal;line-height:1.3">${cls.name}</div><div style="font-size:9px;opacity:.7;margin-top:1px">${cls.room}</div></div>`;}
      html+=`<div style="border-right:0.5px solid var(--bd);border-top:0.5px solid var(--bd);height:${CH}px;position:relative">${inner}</div>`;
    }
  }
