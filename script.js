...
function persist(){sv('pl_ev',events);sv('pl_td',todos);sv('pl_mm',memos);sv('pl_cls',classes);sv('pl_asgn',assignments);sv('pl_nid',nid)}
...
function getCls(d){const dow=d.getDay(),ds=fmt(d);return classes.filter(c=>c.days.includes(dow)&&(!c.startDate||ds>=c.startDate)&&(!c.endDate||ds<=c.endDate)&&!(c.excludeDates&&c.excludeDates.includes(ds)))}
...
function switchPage(p){
  ...
  const pmap={cal:'pageCal',school:'pageSchool',opts:'pageOpts'};
  const bmap={cal:'bniCal',school:'bniSchool',opts:'bniOpts'};
...
/* ══ WEEKLY ══ */
function openClsWeekly(dateStr,id){
  const cls=classes.find(c=>c.id===id);if(!cls)return;
  // 날짜 파싱
  const parts=dateStr.split('-').map(Number);
  const dateObj=new Date(parts[0],parts[1]-1,parts[2]);
  const dateLabel=(parts[1])+'월 '+parts[2]+'일';
  mkSheet('clsWeekOv',`<div class="sheet" onclick="event.stopPropagation()">
    <div class="shhdl"></div>
    <div class="shtitle">${cls.name}</div>
    <div style="font-size:13px;color:var(--tx2);margin-bottom:16px">${dateLabel} 수업</div>
    <div class="shbtns" style="flex-direction:column;gap:10px">
      <button class="sbtn" style="font-size:14px" onclick="clsSkipDate('${dateStr}',${id})">이 날 수업만 숨기기</button>
      <button class="sbtn sbtn-del" style="font-size:14px" onclick="closeSheet('clsWeekOv');deleteCls(${id})">수업 전체 삭제</button>
      <button class="sbtn" style="font-size:14px;background:var(--bg-info);border-color:var(--bd-info);color:var(--tx-info)" onclick="closeSheet('clsWeekOv');openClsModal(${id})">수업 수정</button>
      <button class="sbtn" onclick="closeSheet('clsWeekOv')">취소</button>
    </div>
  </div>`);
}
function clsSkipDate(dateStr,id){
  const idx=classes.findIndex(c=>c.id===id);if(idx<0)return;
  const excl=(classes[idx].excludeDates||[]).concat(dateStr);
  classes[idx]={...classes[idx],excludeDates:excl};
  persist();closeSheet('clsWeekOv');renderWeekly();renderTT();
}

function wdcClick(e,dayStr,hr){
  // 클릭 지점에 wev 요소가 있으면 셀 클릭 무시
  const els=document.elementsFromPoint(e.clientX,e.clientY);
  for(let i=0;i<els.length;i++){
    if(els[i]===e.currentTarget)break; // wdc에 도달하면 중단
    if(els[i].classList&&els[i].classList.contains('wev'))return;
  }
  openEvModalAt(dayStr,hr);
}
...
function renderWeekly(){
  ...
  // 날짜별 겹침 레이아웃 미리 계산 (hr루프 밖)
  const _dayLayouts=Array.from({length:7},(_,d)=>{
    const day=addDays(ws,d);
    const evs=getEvs(day).map(e=>({...e,_type:'ev',startMin:(e.startH-SH())*60+e.startM,endMin:(e.endH-SH())*60+e.endM}));
    const cls=getCls(day).map(e=>({...e,_type:'cls',startMin:(e.startH-SH())*60+e.startM,endMin:(e.endH-SH())*60+e.endM}));
    return calcOverlapLayout([...evs,...cls]);
  });
  // 시간 행
  for(let hr=SH();hr<EH();hr++){
    ...
    for(let d=0;d<7;d++){
      ...
      let eh='';
      const _wAll=_dayLayouts[d];
      _wAll.filter(e=>e.startH===hr).forEach(function(e){
        ...
        const locHtml=isDashed
          ?(S.showLocation&&e.room?'<div style="font-size:9px;opacity:.7">'+e.room+'</div>':'')
          :(e.location?'<div style="font-size:9px;opacity:.7">'+e.location+'</div>':'');
        const dispCol=e.color;
        const colW=100/e.totalCols;
        const lp=e.col*colW,rp=100-(e.col+1)*colW;
        const style='height:'+hp+'px;top:'+topOffset+'px;position:absolute;'
          +'left:calc('+lp+'% + 1px);right:calc('+rp+'% + 1px);'
          +'background:'+h2r(dispCol,.18)+';color:'+dispCol+';border-left-color:'+dispCol+';'
          +(isDashed?'border-left-style:dashed;':'');
        const clk=isDashed
          ?'onclick="event.stopPropagation();openClsWeekly(\''+dayStr+'\','+e.id+')"'
      ...
      const dcClk='onclick="wdcClick(event,\'' +dayStr+ '\',' +hr+ ')"';
      h+='<div class="wdc" '+dcClk+' style="position:relative">'+eh+'</div>';
    }
  }
  h+='</div>';
...
function fillDayTodo(){
  ...
  if(active.length){
    active.forEach(t=>{
        items+=`<div class="day-ti-wrap" id="dtw${t.id}" ontouchstart="dtSwipeStart(event,${t.id})" ontouchmove="dtSwipeMove(event,${t.id})" ontouchend="dtSwipeEnd(event,${t.id})" oncontextmenu="return false">
        <div class="day-ti-del" onclick="deleteTodoDay(${t.id})">🗑️</div>
        <div class="day-ti" onclick="toggleTodoDay(${t.id})" onlongpress="dtLongPress(${t.id})">
  ...
  if(done.length){
    items+=`<div class="day-sec-lbl">완료 (${done.length})</div>`;
        ...
        items+=`<div class="day-ti-wrap" id="dtw${t.id}" ontouchstart="dtSwipeStart(event,${t.id})" ontouchmove="dtSwipeMove(event,${t.id})" ontouchend="dtSwipeEnd(event,${t.id})" oncontextmenu="return false">
        <div class="day-ti-del" onclick="deleteTodoDay(${t.id})">🗑️</div>
        <div class="day-ti" onclick="toggleTodoDay(${t.id})">
...
/* ══ DAY TODO SWIPE & LONGPRESS ══ */
let _dtSwipe={};
let _dtLongTimer=null;

function dtSwipeStart(e,id){
  const t=e.touches[0];
  _dtSwipe={id,startX:t.clientX,startY:t.clientY,moved:false};
  // 롱프레스 타이머
  _dtLongTimer=setTimeout(()=>{
    if(!_dtSwipe.moved){dtLongPress(id);}
  },500);
}
function dtSwipeMove(e,id){
  if(!_dtSwipe||_dtSwipe.id!==id)return;
  const t=e.touches[0];
  const dx=t.clientX-_dtSwipe.startX;
  const dy=Math.abs(t.clientY-_dtSwipe.startY);
  if(dy>10){clearTimeout(_dtLongTimer);_dtSwipe.moved=true;return;}
  if(dx<-5){_dtSwipe.moved=true;clearTimeout(_dtLongTimer);}
  if(dx<0){
    e.preventDefault();
    const wrap=document.getElementById('dtw'+id);
    if(wrap){
      const inner=wrap.querySelector('.day-ti');
      const reveal=Math.min(-dx,72);
      if(inner)inner.style.transform='translateX('+(-reveal)+'px)';
    }
  }
}
function dtSwipeEnd(e,id){
  clearTimeout(_dtLongTimer);
  const wrap=document.getElementById('dtw'+id);
  const inner=wrap?wrap.querySelector('.day-ti'):null;
  if(!_dtSwipe||_dtSwipe.id!==id){if(inner)inner.style.transform='';return;}
  const dx=(_dtSwipe.startX)-(e.changedTouches[0].clientX);
  if(dx>50){
    // 충분히 스와이프 → 빨간 영역 열어두기
...
function dtLongPress(id){
  // 진동
  if(navigator.vibrate)navigator.vibrate(30);
  // 현재 열린 스와이프 닫기
  const wrap=document.getElementById('dtw'+id);
  if(wrap){const inner=wrap.querySelector('.day-ti');if(inner)inner.style.transform='';}
  const t=todos.find(x=>x.id===id);
  if(!t)return;
  mkSheet('dtMenuSh',`<div class="sheet" onclick="event.stopPropagation()">
    <div class="shhdl"></div>
    <div class="shtitle" style="font-size:15px;margin-bottom:12px">${t.text}</div>
    <div class="shbtns" style="flex-direction:column;gap:10px">
      <button class="sbtn" style="font-size:14px" onclick="dtEditTodo(${id})">수정</button>
      <button class="sbtn sbtn-del" style="font-size:14px" onclick="deleteTodoDay(${id});closeSheet('dtMenuSh')">삭제</button>
      <button class="sbtn" onclick="closeSheet('dtMenuSh')">취소</button>
    </div>
  </div>`);
}
function dtEditTodo(id){
  closeSheet('dtMenuSh');
  const t=todos.find(x=>x.id===id);
  if(!t)return;
  mkSheet('dtEditSh',`<div class="sheet" onclick="event.stopPropagation()">
    <div class="shhdl"></div>
    <div class="shtitle">할 일 수정</div>
    <div class="fr"><label>내용</label><input id="dtEditInp" value="${t.text}" inputmode="text"></div>
    <div class="shbtns">
      <button class="sbtn" onclick="closeSheet('dtEditSh')">취소</button>
      <button class="sbtn sbtn-save" onclick="dtSaveTodo(${id})">저장</button>
    </div>
  </div>`);
}
function dtSaveTodo(id){
  const inp=document.getElementById('dtEditInp');
  if(!inp||!inp.value.trim())return;
  const t=todos.find(x=>x.id===id);
  if(t){t.text=inp.value.trim();persist();}
  closeSheet('dtEditSh');fillDayTodo();
}
function deleteTodoDay(id){
  todos=todos.filter(t=>t.id!==id);persist();fillDayTodo();
}
...
function addDayTodo(){
  ...
  if(!inp||!inp.value.trim())return;
  todos.push({id:nid++,text:inp.value.trim(),done:false,date:fmt(curDate),catId:0,repeat:{on:false,type:'none',days:[],until:''}});
  inp.value='';
  persist();
  fillDayTodo();
}
...
/* ══ OVERLAP LAYOUT ══ */
function calcOverlapLayout(items){
  if(!items.length)return[];
  // startMin 순 정렬
  const res=items.map(e=>({...e,col:0,totalCols:1}))
    .sort((a,b)=>a.startMin-b.startMin);
  const n=res.length;

  // 겹침 여부: 시작/끝이 딱 맞닿는 건 겹침 아님 (-1 마진)
  function overlaps(a,b){return a.startMin<b.endMin-1&&b.startMin<a.endMin-1;}

  // 1패스: 각 항목에 컬럼 배정 (앞에서 이미 배정된 것들 기준)
  for(let i=0;i<n;i++){
    const usedCols=[];
    for(let j=0;j<i;j++){
      if(overlaps(res[i],res[j]))usedCols.push(res[j].col);
    }
    let col=0;
  ...
  // 2패스: 겹치는 그룹별로 totalCols 계산
  // 각 항목에 대해 동시에 겹치는 모든 항목(앞뒤 모두) 중 최대 col+1
  for(let i=0;i<n;i++){
    let maxCol=res[i].col;
    for(let j=0;j<n;j++){
      if(i!==j&&overlaps(res[i],res[j])){
        if(res[j].col>maxCol)maxCol=res[j].col;
      }
    }
    res[i].totalCols=maxCol+1;
  }
  return res;
}
...
function fillTGrid(){
  ...
  // 일반 일정 + 수업 겹침 레이아웃
  const _dayEvs=getEvs(curDate).map(ev=>({...ev,_type:'ev',startMin:(ev.startH-SH())*60+ev.startM,endMin:(ev.endH-SH())*60+ev.endM}));
  const _dayCls=getCls(curDate).map(cls=>({...cls,_type:'cls',startMin:(cls.startH-SH())*60+cls.startM,endMin:(cls.endH-SH())*60+cls.endM}));
  const _allDay=calcOverlapLayout([..._dayEvs,..._dayCls]);
  for(const e of _allDay){
    const top=e.startMin/60*HH,h=Math.max((e.endMin-e.startMin)/60*HH,28);
    const colW=100/e.totalCols;
    const leftPct=e.col*colW,rightPct=100-(e.col+1)*colW;
    const leftStyle='left:calc('+leftPct+'% + 1px);right:calc('+rightPct+'% + 1px)';
    if(e._type==='ev'){
      html+=`<div class="evb" id="ev${e.id}" style="top:${top}px;min-height:${h}px;${leftStyle};background:${h2r(e.color,.14)};border-left-color:${e.color}" ontouchstart="tStart(event,${e.id})" onclick="openEvEdit(${e.id})"><div class="evtitle" style="color:${e.color}">${e.title}</div><div class="evsub" style="color:${e.color}">${pad(e.startH)}:${pad(e.startM)}–${pad(e.endH)}:${pad(e.endM)}${e.location?' · '+e.location:''}</div></div>`;
    } else {
      const clsDispCol=e.color;
      html+=`<div class="evb" style="top:${top}px;min-height:${h}px;${leftStyle};background:${h2r(clsDispCol,.3)};border-left-color:${clsDispCol};border-left-style:dashed" onclick="openClsModal(${e.id})"><div class="evtitle" style="color:${clsDispCol}">${e.name}</div>${S.showLocation?`<div class="evsub" style="color:${clsDispCol}">${e.room} · ${e.professor}</div>`:''}</div>`;
    }
  }
...
function renderAsgn(){
  ...
  // 정렬: 과목명 ㄱㄴㄷ → 과목 내 과제 제목 ㄱㄴㄷ
  function sortAsgn(arr){
    return [...arr].sort((a,b)=>{
      const ca=classes.find(c=>c.id===a.classId),cb=classes.find(c=>c.id===b.classId);
      const na=ca?ca.name:'',nb=cb?cb.name:'';
      if(na!==nb)return na.localeCompare(nb,'ko');
      return a.title.localeCompare(b.title,'ko');
    });
  }
  // 마감일 없는 과제 분리 → 진행 중 맨 아래
  const activeWithDL=active.filter(a=>a.deadline);
  const activeNoDL=active.filter(a=>!a.deadline);
  let html='';
  sortAsgn(activeWithDL).forEach(a=>{html+=asgnHTML(a,today)});
  if(activeNoDL.length){
    html+=`<div class="asec">마감일 미정</div>`;
    sortAsgn(activeNoDL).forEach(a=>{html+=asgnHTML(a,today)});
  }
  if(done.length){
    html+=`<div class="asec" style="margin-top:8px">완료된 과제 (${done.length})</div>`;
    sortAsgn(done).forEach(a=>{html+=asgnHTML(a,today)});
  }
...
function bdg(dl,today){
  ...
  if(diff<0)return`<span class="badge badge-ok">기한 지남</span>`;
...
function asgnHTML(a,today){
  ...
  const _dotCol=(()=>{const _c=classes.find(c=>c.id===a.classId);return _c?_c.color:'#A30B2E';})();
  return`<div class="ai ${a.done?'ai-done':''}" id="ai${a.id}">
    <div class="ai-top">
      <div class="ai-check ${a.done?'done':''}" onclick="toggleAsgnDone(${a.id})" style="border-color:${_dotCol}${a.done?';background:'+_dotCol:''}">
        ${a.done?'<svg width="12" height="12" viewBox="0 0 14 14" fill="none"><path d="M2.5 7L5.5 10.5L11.5 3.5" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>':''}
      </div>
      <div class="ai-title ${a.done?'done':''}">${a.title}</div>
...
function renderOpts(){
  ...
  content.innerHTML=`
    ...
    <div class="optrow">
      <span class="optlbl">기본 달력 뷰</span>
      <div style="display:flex;border:0.5px solid var(--bd);border-radius:var(--r);overflow:hidden">
        ${['month','week','day'].map((v,i)=>{
          const labels=['월간','주간','일간'];
          const isActive=S.defaultView===v;
          return '<button onclick="S.defaultView=\''+v+'\';saveS();renderOpts()" style="padding:7px 12px;border:none;background:'+(isActive?'var(--bg-info)':'transparent')+';color:'+(isActive?'var(--tx-info)':'var(--tx2)')+';font-size:13px;cursor:pointer;font-weight:'+(isActive?'500':'400')+';font-family:var(--font);border-right:'+(i<2?'0.5px solid var(--bd)':'none')+'">'+labels[i]+'</button>';
        }).join('')}
      </div>
    </div>
...
const ONBOARDING_SLIDES = [
  ...
  },
  {
    ...
    items: [
      ...
      {t:'부속 할 일', d:'과제마다 세부 할 일을 추가하고 날짜등록 버튼으로 일간 뷰 할 일에 바로 연결할 수 있어요. 과제에서 체크하면 일간 뷰에도 자동 반영돼요.'},
    ]
  },
  {
    icon: '⚙️',
...
function resetAll(){if(confirm('모든 데이터를 초기화할까요?\n이 작업은 되돌릴 수 없습니다.')){events=[];todos=[];memos={};classes=[];assignments=[];nid=1;persist();renderMonthly();fillTGrid();fillMemo();renderSchool();alert('초기화 완료!')}}
...
function exportData(){
  const backup={
    ...
    events,todos,memos,classes,assignments,nid,settings:S
  };
...
function showEvSheet(ev){
  ...
  mkSheet('evSh',`<div class="sheet" onclick="event.stopPropagation()">
    ...
    <div class="fr"><label>장소</label><input id="eLocation" value="${ev.location||''}" placeholder="장소 (선택)" inputmode="text"></div>
...
function afterEvSave(){persist();closeSheet('evSh');fillTGrid();if(calView==='month')renderMonthly();else if(calView==='week')renderWeekly();}
function afterClsSave(){persist();closeSheet('clsSh');renderTT();fillTGrid();if(calView==='week')renderWeekly();}
function deleteEv(id){
  const ev=events.find(e=>e.id===id);
  if(ev&&ev.repeat&&ev.repeat.on){openRepeatDeleteScope(id);}
  else{events=events.filter(e=>e.id!==id);afterEvSave();}
}
...
function saveEv(id){
  ...
  const location=(document.getElementById('eLocation')||{value:''}).value.trim();
  const data={title,location,date:dateVal,startH:allDay?0:sH,startM:allDay?0:sM,endH:allDay?0:eH,endM:allDay?0:eM,color:evColor,showM,allDay,repeat:{on:repOn,weeks,days,until}};
...
function applyRepeatEdit(id,scope){
  ...
  if(scope==='all'){
    ...
    // 이 일정만 → 기존 반복에서 이 날 제외(excludes) + 단일 수정본 추가
    const excl=(orig.repeat.excludes||[]).concat(todayStr);
    events[origIdx]={...orig,repeat:{...orig.repeat,excludes:excl}};
...
function deleteCls(id){classes=classes.filter(c=>c.id!==id);afterClsSave();}
function saveCls(id){
...
function confirmRegTodo(aid,tid){
  ...
  if(t.linkedTodoId){
    ...
    if(existing){existing.date=dateVal;existing.catId=catId;persist();closeSheet('regTodoSh');renderAsgn();return;}
  }
  // 새 투두 생성
  const newTodo={
    id:nid++,
