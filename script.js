...
function renderTT(){
  ...
  for(let d=1;d<=5;d++){
    ...
    dayCls.forEach(cls=>{
      const startMin=cls.startH*60+cls.startM;
      const endMin=cls.endH*60+cls.endM;
      const top=(startMin - minH*60)/60*CH;
      const blockH=(endMin-startMin)/60*CH - 2;
      colInner+=`<div class="ttcls" style="top:${top}px;height:${blockH}px;background:${h2r(cls.color,.15)};border-left-color:${cls.color};color:${cls.color}" onclick="openClsModal(${cls.id})">
...
const ONBOARDING_SLIDES = [
  {
    ...
    title: '달력',
    items: [
      {t:'월간 뷰', d:'한 달 일정을 한눈에 확인해요. 날짜를 탭하면 그날 일정 목록과 일정 추가 버튼이 나타나요. 메모가 있는 날은 빨간 점으로 표시돼요.'},
      {t:'주간 뷰', d:'이번 주 전체 흐름을 타임블록으로 확인해요. 빈 칸을 탭하면 바로 일정을 추가할 수 있어요. 수업도 점선 블록으로 함께 표시돼요.'},
      {t:'일간 뷰', d:'왼쪽엔 시간대별 일정, 오른쪽엔 그날의 할 일이 함께 표시돼요. 일정 블록을 길게 눌러 드래그하면 15분 단위로 시간을 조정할 수 있어요.'},
      {t:'반복 일정', d:'요일·주 단위로 반복되는 일정을 등록할 수 있어요. 수정 시 이 일정만 / 이후 전체 / 모든 반복 중 범위를 선택할 수 있어요.'},
    ]
  },
  {
    icon: '✅',
  ...
  },
  {
    ...
    title: '학교',
    items: [
      {t:'시간표', d:'수업명·교수·강의실·개강/종강일·요일·시간을 등록하면 일간·주간 뷰에 자동으로 표시돼요.'},
      {t:'과제 / 시험', d:'과제는 반드시 수업에 연결해서 등록해요. 마감일과 D-day 뱃지로 남은 시간을 한눈에 확인할 수 있어요.'},
      {t:'부속 할 일', d:'과제마다 세부 할 일을 추가하고 날짜등록 버튼으로 할 일 탭에 바로 연결할 수 있어요.'},
    ]
  },
  {
    icon: '⚙️',
    ...
    items: [
      ...
      {t:'폰트 & 테마', d:'Pretendard 등 9종 폰트와 라이트·다크·자동 테마를 선택할 수 있어요.'},
      {t:'기타 설정', d:'주 시작 요일, 시간 범위, 기본 캘린더 뷰, 타임블록 장소 표시 여부를 설정할 수 있어요.'},
      {t:'데이터 저장', d:'모든 데이터는 이 기기에만 저장돼요. 인터넷 없이도 사용할 수 있어요. (폰트 로딩은 인터넷 필요)'},
    ]
  }
];
let _obIdx = 0;
...
function buildColorPicker(rowId, selColor, defSet){
  ...
  function mkDot(c, inAll){
    ...
    return '<div class="cpdot'+(c.h===curColor?' on':'')+'" '
      +'style="background:'+_rgb+';'+(c.h===curColor?'transform:scale(1.2)':'')+'" '
      +'onclick="'+fn+'" '
      ...
      if(mainRow){
        ...
        mainRow.innerHTML = pal.map(function(c){
          ...
          return '<div class="cpdot'+(c.h===color?' on':'')+'" '
            +'style="background:'+_rgb2+';'+(c.h===color?'transform:scale(1.2)':'')+'" '
